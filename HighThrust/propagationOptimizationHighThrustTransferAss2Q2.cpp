...
int main( )
{
    ...
    std::string outputPath = tudat_applications::getOutputPath( "HighThrustAss2Q2" );
    ...
    double radiationPressureCoefficient = 1.0; // AANPASSEN
    ...
    std::vector<double> errorList = {0.000078,0.0020,0.02,0.0065,0.016,0.029,1.3,0.1};
    std::vector<std::string> bodyList = {"Sun","Earth","Venus","Mars","Jupiter","Saturn","Uranus"};
    unsigned int prange;
    unsigned int erange;
    ...
    for( unsigned int k = 0; k <9; k++){
        ...
        int x = 0;
        if( k> 0 && k < 8){
            gravityParameter = spice_interface::getBodyGravitationalParameter(bodyList.at(k-1));
        }
        ...
        for( unsigned int e = 0; e<erange; e++){
            ...
            for( unsigned int p = 0; p<prange; p++){
                ...
                if( k> 0 && k < 8){
                    //bodyMapForPatchedConic[ bodyList.at(k-1) ].reset();

                    bodyMapForPatchedConic[ bodyList.at(k-1) ]->setGravityFieldModel(
                                createGravityFieldModel(
                                    std::make_shared< CentralGravityFieldSettings >(
                                        gravityParameter ),  bodyList.at(k-1)  ) );
                    error = spice_interface::getBodyGravitationalParameter(bodyList.at(k-1))*randomNumberGenerator()/1000000.0;
                    double newParameter = spice_interface::getBodyGravitationalParameter(bodyList.at(k-1))+error;
                    bodyMapForPatchedConic[bodyList.at(k-1)]->setGravityFieldModel(
                                createGravityFieldModel(
                                    std::make_shared< CentralGravityFieldSettings >(
                                        newParameter), bodyList.at(k-1) ) );
                }
                else if (k ==8) {
                    double referenceAreaRadiation = 51; // AANPASSEN
                    double radiationPressureCoefficient = 1.0+randomNumberGenerator(); // AANPASSEN
                    std::shared_ptr< RadiationPressureInterfaceSettings > JUPRadiationPressureSettings =
                            std::make_shared< CannonBallRadiationPressureInterfaceSettings >(
                                "Sun", referenceAreaRadiation, radiationPressureCoefficient);
                ...
                double relativeTolerance = 1E-9;//reset to -9
                double absoluteTolerance = 1E-9;//reset to -9
                ...
                if( k == 0){
                    ...
                    for (auto resultIterator : fullProblemResultForEachLeg){
                        ...
                        //                                    std::to_string( resultIterator.first ) + "reference_fullProblemComparison"+ std::to_string(k)+std::to_string(e)+std::to_string(p)+"Interpolated" + ".dat", outputPath );
                        i = i+1;
        ...
        input_output::writeDataMapToTextFile(
                    MaxPosError, "PositionResultQ2PerScenario" +
                    std::to_string(k)+".dat", outputPath );
        input_output::writeDataMapToTextFile(
                    MaxVelError, "VelocityResultQ2PerScenario" +
                    std::to_string(k)+".dat", outputPath );
    }
